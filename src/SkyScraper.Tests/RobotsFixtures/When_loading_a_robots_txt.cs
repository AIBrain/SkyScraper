...
// 
// "SkyScraper.Tests/When_loading_a_robots_txt.cs" was last cleaned by Rick on 2014/07/06 at 4:00 PM
#endregion
...
namespace SkyScraper.Tests.RobotsFixtures {
    ...
    [TestFixture]
    internal class When_loading_a_robots_txt {
        [TestFixtureSetUp]
        ...
        public void Then_login_should_not_be_allowed() {
            Robots.PathIsAllowed( "/login.html?ok=1" )
                  .Should()
                  .BeFalse();
        }
        ...
        public void Then_sitemap_should_be_set() {
            Robots.SiteMap.Should()
                  .Be( "http://foo/sitemap.xml" );
        }
    }
}
